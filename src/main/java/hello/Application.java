...
public class Application {
	...
	public String index(@RequestBody ArenaUpdate arenaUpdate) {
		...
		for (Map.Entry<String, PlayerState> state : arenaUpdate.arena.state.entrySet()) {
			...
			if (debug) {
				System.out.println("Player " + playerCount + " (me " + isMe + ") URL " + url + " pos" + playerState.x
						+ "," + playerState.y);
			}
	...
	private Vector2 detect(PlayerState me, String direction) {
		...
		switch (direction) {
		case "N":
			targets = new Vector2[] { new Vector2(me.x, me.y - 1), new Vector2(me.x, me.y - 2),
					new Vector2(me.x, me.y - 3) };
			break;
		case "W":
			targets = new Vector2[] { new Vector2(me.x - 1, me.y), new Vector2(me.x - 2, me.y),
